...
export function EventForm({ onSave, onClose, initial }: EventFormProps) {
    ...
    const [lunarYear, setYear] = useState(new Date().getFullYear());
    ...
    async function handleSave() {
        ...
        setSaving(true);
        await onSave({ title: title.trim(), description, lunarDay, lunarMonth, isLeapMonth: isLeap, repeatYearly, notifyEnabled, notifyDaysBefore: notifyDays });
        setSaving(false);
        onClose();
    }
        ...
        <>
            <div className="fixed inset-0 z-40 flex items-end justify-center">
                ...
                <div className="relative w-full max-w-[480px] rounded-t-2xl shadow-lg pb-8"
                    ...
                    <div className="flex items-center justify-between px-4 py-3 border-b border-vn">
                        ...
                        <span className="font-display font-semibold text-primary">Thêm Sự Kiện</span>
                        <button
                    ...
                    <div className="px-4 pt-4 flex flex-col gap-4">
                        ...
                        {/* Ngày âm lịch */}
                        <div>
                            <label className="text-xs font-semibold text-muted-vn uppercase tracking-wide mb-1 block">
                                Ngày Âm Lịch
                            </label>
                            <button
                        ...
                        {/* Ghi chú */}
                        <div>
                            <label htmlFor="event-desc" className="text-xs font-semibold text-muted-vn uppercase tracking-wide mb-1 block">
                                Ghi Chú (tùy chọn)
                            </label>
                            <textarea
                                id="event-desc"
            ...
            {showPicker && (
                <LunarDatePicker
                    ...
                    initialDay={lunarDay}
                    onConfirm={(y, m, d, leap) => { setYear(y); setMonth(m); setDay(d); setIsLeap(leap); setShowPicker(false); }}
